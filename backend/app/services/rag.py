...
async def ingest_text(
    ...
    *,
    ...
    doc_id = scoped_doc_id(user_id, doc_id or new_doc_id())
    chunks = chunk_text(text, settings.chunk_size, settings.chunk_overlap)
    count = insert_chunks(doc_id, source, chunks, settings, user_id=user_id)
...
async def rag_answer(
    ...
    *,
    ...
    contexts = search_similar(question, top_k=top_k, settings=settings, user_id=user_id)
...
def remove_document(doc_id: str, user_id: str, settings: Settings | None = None) -> None:
    assert_doc_owned(user_id, doc_id)
    delete_document(doc_id, settings, user_id=user_id)


def get_documents(user_id: str, settings: Settings | None = None) -> list[dict[str, Any]]:
    return list_documents(settings, user_id=user_id)
